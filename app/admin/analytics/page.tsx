...
export default function AdminAnalyticsPage() {
  ...
  const barChartData = hotspots.slice(0, 5).map((h) => ({
    name: tLocation(h.point_name).split(' ')[0] + '...',
    fullName: tLocation(h.point_name),
  ...
  const columns: Column<HotspotAnalysisResult>[] = [
    {
      header: t('analyticsPointWardHeader'),
      accessor: (row) => (
        <div>
          <p className="font-extrabold text-slate-900 dark:text-white text-xs">{tLocation(row.point_name)}</p>
          <p className="text-[10px] text-slate-400">{tLocation(row.area_name)}</p>
        </div>
      ),
    },
    {
      header: t('analyticsScoreHeader'),
      accessor: (row) => (
    ...
    {
      header: t('analyticsReportsFillHeader'),
      accessor: (row) => (
    ...
    {
      header: t('analyticsFrequencyHeader'),
      accessor: (row) => (
        <div className="text-xs font-semibold text-slate-700 dark:text-slate-300">
          <span>{t('analyticsPickupsWk', { count: row.collection_frequency_per_week })}</span>
          {row.overdue_count > 0 && (
            <span className="block text-[10px] text-rose-500 font-bold">
              {t('analyticsOverduePickups', { count: row.overdue_count })}
            </span>
          )}
        </div>
      ),
    ...
    {
      header: t('analyticsFactorsHeader'),
      accessor: (row) => (
        <button
        ...
        >
          ...
          <span>{t('analyticsInspectFactors')}</span>
        </button>
      ),
    },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-3xl bg-gradient-to-r from-slate-900 via-teal-950 to-slate-900 p-6 text-white shadow-xl flex flex-col md:flex-row md:items-center md:justify-between gap-4 border border-slate-800">
        <div>
          <div className="flex items-center space-x-2">
            <span className="px-2.5 py-0.5 rounded-full text-[10px] font-extrabold bg-rose-500/30 text-rose-300 border border-rose-500/40 uppercase tracking-wider flex items-center space-x-1">
              ...
              <span>{t('analyticsHotspotEngine')}</span>
            </span>
            <span className="text-xs text-slate-300 font-medium">{t('analyticsMultiFactor')}</span>
          </div>
          <h1 className="mt-2 text-2xl font-extrabold tracking-tight">
            {t('analyticsHotspotTitle')}
          </h1>
          <p className="mt-1 text-xs text-slate-300 max-w-2xl">
            {t('analyticsHotspotSub')}
          </p>
        </div>
        ...
        <div className="flex items-center space-x-2">
          ...
          >
            ...
            <span>{t('analyticsRecalculate')}</span>
          </button>
          <Link
            href="/admin/routes"
          ...
          >
            ...
            <span>{t('analyticsDispatchPriority')}</span>
          </Link>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title={t('analyticsIdentifiedHotspots')} value={t('analyticsLocationsCount', { count: totalHotspots })} subtitle={t('analyticsCriticalUrgency', { count: criticalCount })} icon={Flame} variant="rose" />
        <StatCard title={t('analyticsHighestScore')} value={`${highestScore} / 100`} subtitle={t('analyticsPanchayatPeakRisk')} icon={AlertTriangle} variant="amber" />
        <StatCard title={t('analyticsCitizenReportsEvaluated')} value={`${totalReports} Reports`} subtitle={t('analyticsHistoricalComplaints')} icon={FileText} variant="purple" />
        <StatCard title={t('analyticsAvgFillRate')} value="84.2%" subtitle={t('analyticsHighAccumulation')} icon={TrendingUp} variant="emerald" />
      </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-sm font-extrabold text-slate-900 dark:text-white flex items-center space-x-2">
                ...
                <span>{t('analyticsTopScoresVsDemand')}</span>
              </h3>
              <p className="text-[11px] text-slate-500 dark:text-slate-400">
                {t('analyticsTopScoresSub')}
              </p>
            </div>
          </div>
        ...
        <div className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 shadow-sm space-y-4">
          <div>
            <h3 className="text-sm font-extrabold text-slate-900 dark:text-white flex items-center space-x-2">
              ...
              <span>{t('analyticsCategoryBreakdown')}</span>
            </h3>
            <p className="text-[11px] text-slate-500 dark:text-slate-400">
              {t('analyticsCategoryBreakdownSub')}
            </p>
          </div>
